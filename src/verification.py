...
testbench="`timescale 1ns/10ps module {testbench_module}();task calculate_OC;input [{outputlength}:0] Q;output integer OC;integer bit_i,incorrect_bits,total_bits;begin total_bits = {outputlength}+1;incorrect_bits = 0;for (bit_i = 0; bit_i < total_bits; bit_i = bit_i + 1) begin if (Q[bit_i] == 0) begin incorrect_bits = incorrect_bits + 1; end end OC = incorrect_bits / total_bits; end endtask integer count,xc,OC,BER; {cir_inputs}reg clk;wire [{outputlength}:0] Q;wire Z;integer file;initial begin file = $fopen(\"{log_path}\", \"w\");clk = 0;forever begin #5 {clock_pins} clk = ~clk;end end initial begin xc=0;repeat ({outerloop}) begin if(xc<=2)begin {{{key_inputs_p}}} ={key_bit_count}'b{key_bit_val};end else begin {{{key_inputs_p}}} =$random;end count=0;xc=xc+1;repeat ({innerloop}) begin {{{cir_inputs_p}}} =$random;#10;if(Z==0) begin count=count+1;end end $fwrite(file, \"KEYINPUTS = %b \\n\", {{{key_inputs_p}}});calculate_OC(Q, OC);BER=count/{innerloop};$fwrite(file, \"OC:, %f BER: \\n\\n\",OC,BER);end $finish;$fclose(file);end {top_module} dut (.Q(Q),.Z(Z),{top_port});endmodule"
