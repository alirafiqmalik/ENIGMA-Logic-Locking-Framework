...
def synthesize_verilog(verilog, flag = "flatten"):
    ...
    # synthesized_verilog=format_verilog(synthesized_verilog,remove_wire=True)
    text_file.close()
...
def synthesize_bench(bench):
    ...
    cmd = """
        /home/alira/FYP/linux/yosys/build/yosys-abc'
...
def submodule_links_extraction(verilog):
    ...
    for match in re.finditer(r"(\w+)\s+(\w+)\s*\((.*?)\);", verilog):
        ...
        input_list = re.findall(r"\.(\w+)\((\w+)\)", inputs_str)
